...
const ViewCart = ({ closeView }: ViewCartProps) => {
  const { cart, removeCart, increment, decrement, clearCart } = useBikeStore();
  ...
  if (isOrderPlaced) {
    return (
      <div className="w-full max-w-2xl p-6 rounded-2xl shadow-2xl text-white text-center">
        <h2 className="text-2xl font-semibold mb-3">Order placed!</h2>
        <p className="text-gray-400 mb-6">
          Thanks for your order. We&apos;ll be in touch with delivery details
          soon.
        </p>

        <button
          className="w-full py-3 rounded-xl bg-green-500 hover:bg-green-600 font-semibold cursor-pointer active:scale-95 duration-75"
